...
- Remove all seed data, live vehicles/maintenance/fuel_log from Supabase
- `useVehicleData` hook, display config keyed by UUID
- Maintenance tab: sortable headers, independent scroll, task history modal, log service modal
- Fuel log tab: stats + fillup modal with Claude Vision
...
| 42 | Notifications re-firing for already-cleared items; grocery notification fires even when pref is off because markNotified ran after addedByMe check instead of before | High | ✅ Fixed — markNotified now called immediately on first encounter; push block also gated on !addedByMe |
| 43 | 24-hr weather strip starts 3-4 hours ahead of current time — Tomorrow.io returns data from top of fetch hour; cached result drifts up to 30 min | Medium | ✅ Fixed — filter out past hours before slicing 24 in fetch-weather.js |
| 44 | 6/7-day forecast shows thermometer icon (🌡 fallback) — Tomorrow.io daily timeline uses weatherCodeMax not weatherCode; field was missing from API request and not read | Medium | ✅ Fixed — added weatherCodeMax to API fields, daily icon uses weatherCodeMax ?? weatherCode |
| 45 | Calendar widget Upcoming section shows past events (e.g. Mar 26/27) — isPast() only used for Today badge, not applied to Upcoming filter | High | ✅ Fixed — upcoming filter now excludes isPast() events |
| 46 | Tomorrow.io pressure field returns inHg on imperial units — displayed as 29 hPa instead of ~1015 hPa | Low | ✅ Fixed — multiply by 33.8639 to convert inHg → hPa |
| 47 | Tomorrow.io free tier returns null precipitationProbability on daily timeline — daily forecast showed 0% for all days | High | ✅ Fixed — switched daily forecast to NWS API (same source as Weather Channel). Hybrid architecture: Tomorrow.io for current + hourly, NWS for 7-day daily |
| 48 | Hourly weather strip time labels generated server-side in UTC — showed 5hrs ahead for CST users | High | ✅ Fixed — server stores raw isoTime only, client formats label using device timezone |
