...
| Core grocery items / pantry staples | Separate curated list of household staples with name, qty, store, cadence note. "Add core items" button bulk-adds anything not already on active grocery list, skipping duplicates and already-checked items. Optional: auto-add every Sunday morning. Editable management screen. Supabase table: `core_grocery_items`. Primary use case: Katelin bulk-adds weekly essentials (6x 2% milk, etc.) in one tap. |
| Screensaver / photo slideshow | Direct photo upload for wall display screensaver mode. Table stakes for a wall-mounted dashboard — all smart display competitors have it. Relevant once wall display (Android 13 touchscreen) is installed. | Start with Flipr (~$180) if pursuing — floats in pool, WiFi, community HA integration exists. Measures pH and ORP (proxy for chlorine, not direct). Caveats: ORP is influenced by pH/temp/cyanuric acid so can mislead without context; sand filter + slow leak means chemistry fluctuates more than a typical pool; probes require recalibration every 1–4 weeks. Real value is catching a mid-week chemistry crash between manual tests, especially after a hot sunny weekend. If pool chemistry is stable week-to-week, ROI is low. Revisit after other sensor sprints are complete. |
